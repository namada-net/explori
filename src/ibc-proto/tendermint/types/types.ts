...
export const CommitSig: MessageFns<CommitSig> = {
  encode(message: CommitSig, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.blockIdFlag !== BlockIDFlag.BLOCK_ID_FLAG_UNKNOWN) {
      writer.uint32(8).int32(blockIDFlagToNumber(message.blockIdFlag));
    }
    if (message.validatorAddress.length !== 0) {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1: {
          ...
          message.blockIdFlag = blockIDFlagFromJSON(reader.int32());
          continue;
        }
        case 2: {
          if (tag !== 18) {
  ...
  fromPartial<I extends Exact<DeepPartial<CommitSig>, I>>(object: I): CommitSig {
    const message = createBaseCommitSig();
    message.blockIdFlag = object.blockIdFlag ?? BlockIDFlag.BLOCK_ID_FLAG_UNKNOWN;
    message.validatorAddress = object.validatorAddress ?? new Uint8Array(0);
    message.timestamp = object.timestamp ?? undefined;
    message.signature = object.signature ?? new Uint8Array(0);
    return message;
