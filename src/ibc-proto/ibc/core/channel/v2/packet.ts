...
export function packetStatusToNumber(object: PacketStatus): number {
  switch (object) {
    case PacketStatus.PACKET_STATUS_UNSPECIFIED:
      return 0;
    case PacketStatus.PACKET_STATUS_SUCCESS:
...
function createBaseRecvPacketResult(): RecvPacketResult {
  return { status: PacketStatus.PACKET_STATUS_UNSPECIFIED, acknowledgement: new Uint8Array(0) };
}
...
export const RecvPacketResult: MessageFns<RecvPacketResult> = {
  encode(message: RecvPacketResult, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.status !== PacketStatus.PACKET_STATUS_UNSPECIFIED) {
      writer.uint32(8).int32(packetStatusToNumber(message.status));
    }
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1: {
          ...
          message.status = packetStatusFromJSON(reader.int32());
          continue;
        }
        case 2: {
          if (tag !== 18) {
  ...
  fromJSON(object: any): RecvPacketResult {
    return {
      status: isSet(object.status) ? packetStatusFromJSON(object.status) : PacketStatus.PACKET_STATUS_UNSPECIFIED,
  ...
  fromPartial<I extends Exact<DeepPartial<RecvPacketResult>, I>>(object: I): RecvPacketResult {
    const message = createBaseRecvPacketResult();
    message.status = object.status ?? PacketStatus.PACKET_STATUS_UNSPECIFIED;
