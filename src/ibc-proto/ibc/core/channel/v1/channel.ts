...
export function orderToNumber(object: Order): number {
  switch (object) {
    case Order.ORDER_NONE_UNSPECIFIED:
      return 0;
    case Order.ORDER_UNORDERED:
...
function createBaseChannel(): Channel {
  return {
    state: State.STATE_UNINITIALIZED_UNSPECIFIED,
...
export const Channel: MessageFns<Channel> = {
  encode(message: Channel, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.state !== State.STATE_UNINITIALIZED_UNSPECIFIED) {
      writer.uint32(8).int32(stateToNumber(message.state));
    }
    if (message.ordering !== Order.ORDER_NONE_UNSPECIFIED) {
      writer.uint32(16).int32(orderToNumber(message.ordering));
    }
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2: {
          ...
          message.ordering = orderFromJSON(reader.int32());
          continue;
        }
        case 3: {
          if (tag !== 26) {
  ...
  fromJSON(object: any): Channel {
    return {
      state: isSet(object.state) ? stateFromJSON(object.state) : State.STATE_UNINITIALIZED_UNSPECIFIED,
      ordering: isSet(object.ordering) ? orderFromJSON(object.ordering) : Order.ORDER_NONE_UNSPECIFIED,
  ...
  fromPartial<I extends Exact<DeepPartial<Channel>, I>>(object: I): Channel {
    const message = createBaseChannel();
    message.state = object.state ?? State.STATE_UNINITIALIZED_UNSPECIFIED;
    message.ordering = object.ordering ?? Order.ORDER_NONE_UNSPECIFIED;
